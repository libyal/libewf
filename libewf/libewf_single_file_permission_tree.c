/*
 * Single file permission tree functions
 */
...
/* Retrieves the single file permission sub node for the specific permission id
 * Returns 1 if successful, 0 if no such sub single file permission or -1 on error
 */
...
{
	static char *function   = "libewf_single_file_permission_tree_get_sub_node_by_id";
	...
	if( sub_single_file_permission == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
		 LIBCERROR_ARGUMENT_ERROR_INVALID_VALUE,
		 "%s: invalid sub single file permission.",
		 function );
	     ...
	     sub_node_index++ )
	{
		if( libcdata_tree_node_get_value(
		     *sub_node,
		     (intptr_t **) sub_single_file_permission,
		     error ) != 1 )
		{
			libcerror_error_set(
			 error,
		...
		if( *sub_single_file_permission == NULL )
		{
			libcerror_error_set(
			 ...
			 "%s: missing sub single file permission: %d.",
			 function,
			 sub_node_index );

			goto on_error;
		}
		if ((*sub_single_file_permission)->id == permission_id)
		{
			break;
		}
	...
	{
		*sub_node               = NULL;
		*sub_single_file_permission = NULL;
