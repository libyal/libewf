         ...
         liberror_error_t **error )
{
	 ...
	 */
	if( ( internal_handle->io_handle->current_chunk_offset != 0 )
	...
	if( segment_file_handle->write_open != 0 )
	{
		...
		if( internal_handle->write_io_handle->chunks_section_offset != 0 )
		{
			...
			if( internal_handle->write_io_handle->amount_of_table_offsets < internal_handle->write_io_handle->section_amount_of_chunks )
			{
				reallocation = memory_reallocate(
				                internal_handle->write_io_handle->table_offsets,
				                sizeof( ewf_table_offset_t ) * internal_handle->write_io_handle->section_amount_of_chunks );
			...
			write_count = libewf_segment_file_write_chunks_section_correction(
				       ...
				       internal_handle->offset_table,
			               internal_handle->write_io_handle->table_offsets,
			               internal_handle->write_io_handle->amount_of_table_offsets,
				       ...
				       internal_handle->write_io_handle->section_amount_of_chunks,
			               error );
			...
			write_count_finalize                          += write_count;
			internal_handle->write_io_handle->write_count += write_count;
		}
		...
		write_count = libewf_segment_file_write_close(
		               ...
		               internal_handle->io_handle,
		               segment_number,
		               internal_handle->write_io_handle->segment_amount_of_chunks,
		               1,
		               internal_handle->hash_sections,
		               internal_handle->hash_values,
		               internal_handle->media_values,
	...
	if( internal_handle->media_values->media_size == 0 )
	{
		 ...
		 */
		internal_handle->media_values->amount_of_chunks  = internal_handle->write_io_handle->amount_of_chunks;
		internal_handle->media_values->amount_of_sectors = (uint32_t) ( internal_handle->write_io_handle->input_write_count
		                                                 / internal_handle->media_values->bytes_per_sector );
		internal_handle->media_values->media_size        = (size32_t) internal_handle->write_io_handle->input_write_count;
		...
		for( segment_table_iterator = 1; segment_table_iterator < internal_handle->segment_table->amount; segment_table_iterator++ )
		{
			...
			while( list_element != NULL )
			{
				     ...
				     6 ) == 0 )
				{
					...
					else if( internal_handle->io_handle->ewf_format == EWF_FORMAT_E01 )
					{
