...
{ 
	...
	${TEST_RUNNER} ${TMPDIR} ${TEST_EXECUTABLE} -C Case -D Description -E Evidence -e Examiner -m removable -M logical -N Notes -q -t ${TMPDIR}/${INPUT_NAME}.acquirestream ${OPTIONS[*]} < ${INPUT_FILE} | sed '1,2d' | sed "s?${TMPDIR}/??" > ${TEST_RESULTS};
	...
	if test ${RESULT} -eq ${EXIT_SUCCESS};
        then
                ${VERIFY_TOOL} -q ${TMPDIR}/${INPUT_NAME}.acquirestream.* > /dev/null;
...
run_tests "${TEST_PREFIX}acquirestream" "${TEST_PREFIX}acquirestream" "${ACQUIRE_TOOL}";
