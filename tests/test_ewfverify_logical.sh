...
{ 
	...
	${TEST_RUNNER} ${TMPDIR} ${TEST_EXECUTABLE} -f files -q ${OPTIONS} ${INPUT_FILE} | sed '1,2d' > ${TEST_RESULTS};
...
run_tests "${TEST_PREFIX}verify_logical" "${TEST_PREFIX}verify_logical" "${VERIFY_TOOL}";
