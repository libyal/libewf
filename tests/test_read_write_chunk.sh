...
{ 
	...
	${TEST_RUNNER} ${TMPDIR} ${TEST_EXECUTABLE} -t ${TMPDIR}/read_write ${OPTIONS} ${INPUT_FILE};
...
TEST_READ_WRITE="./${TEST_EXECUTABLE}";

if ! test -x "${TEST_READ_WRITE}";
then
	TEST_READ_WRITE="${TEST_EXECUTABLE}.exe";
fi

if ! test -x "${TEST_READ_WRITE}";
then
	echo "Missing executable: ${TEST_READ_WRITE}";
